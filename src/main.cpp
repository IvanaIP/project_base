...
auto main() -> int {
  ...
  Shader carLineShader("resources/shaders/carline.vs",
                       "resources/shaders/carline.fs");
  Shader carShader("resources/shaders/default.vs",
                   "resources/shaders/default.fs");
  Shader villaShader("resources/shaders/villa.vs",
                     "resources/shaders/villa.fs");
  Shader clockShader("resources/shaders/clock.vs",
                     "resources/shaders/clock.fs");
  Shader floorShader("resources/shaders/default.vs",
                     "resources/shaders/default.fs");

  Model villaModel(
      "resources/objects/futuristic_app/Futuristic\ Apartment.obj");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // pozicija svetla
    pointLight.position =
        glm::vec3(150.0 * cos(currFrame), 120 + 100.0f * abs(cos(currFrame)),
                  150 * sin(currFrame / 10));
    ...
    car_model           = glm::translate(
                  car_model, programState->backpackPosition + glm::vec3(0, 0, 45));
    ...
    for (int i = 0; i < 101; i++) {
      ...
      clock_model           = glm::translate(
                    clock_model,
                    programState->backpackPosition +
                        glm::vec3(
                            cos(i * currentFrame) * ((i + 1) * currentFrame),
                            10 + sin(currentFrame * 20) * 2 * cos(currentFrame * 20),
                            35 * sin(currentFrame * i + 5)));
      clock_model =
          glm::scale(clock_model, glm::vec3(programState->backpackScale));
    ...
    floor_model = glm::translate(floor_model, programState->backpackPosition +
                                                  glm::vec3(0.0f, -2.0f, 0.0f));
    floor_model =
        glm::scale(floor_model, glm::vec3(programState->backpackScale * 5));
    ...
    grass_model = glm::translate(grass_model, programState->backpackPosition +
                                                  glm::vec3(0.0f, -2.0f, 0.0f));
    grass_model =
        glm::scale(grass_model, glm::vec3(programState->backpackScale * 5));
    ...
    villa_model           = glm::translate(
                  villa_model, programState->backpackPosition + glm::vec3(0, 0, 0));
    villa_model =
        glm::scale(villa_model, glm::vec3(programState->backpackScale));
